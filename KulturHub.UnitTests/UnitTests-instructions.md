...
> **Note**: This document describes the intended testing strategy for the entire solution.
> Currently, `KulturHub.UnitTests` exists as a project (xUnit, Moq, FluentAssertions, coverlet), but contains **no concrete tests yet**. As soon as additional use cases (e.g. `AuthService.SignUpAsync`) are implemented, the conventions and required test cases documented here will be applied.
...
**Rules**:
- One test class per handler: `{HandlerName}Tests.cs`
- Repositories are always mocked (never a real DB)
- At the top of each test class, list all rules as a comment, then write the tests
